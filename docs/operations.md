...
`granite-dev-generator-plane` is centered on developer tools work in Rust. The technical target is: Build a Rust toolkit that studies generator behavior through node-edge fixtures, with cycle and reachability reports and local-only command execution.
...
The score is deterministic. The current threshold is 159. Risk has a penalty of 6, latency has a penalty of 2, and weight contributes a bonus of 2.
